...
class LeftMemoViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    // Tableで使用する配列を定義する.
    private let myiPhoneItems: NSArray = ["メモ","ゴミ箱"]
    private let myAndroidItems: NSArray = ["5.x", "4.x", "2.x", "1.x"]
    
    // Sectionで使用する配列を定義する.
    private let mySections: NSArray = ["ファイル", "タグ"]
    ...
    override func viewDidLoad() {
        ...
        let myTableView: UITableView = UITableView(frame: CGRect(x: 0, y: 0, width: 200, height: displayHeight - barHeight - 49 - 44))
